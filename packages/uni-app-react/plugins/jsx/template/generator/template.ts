...
export class BaseTemplate {
  public _baseLevel = 0
  public _isUseXS = true
  public _isUseCompileMode = false
  public exportExpr = 'module.exports ='
  public isSupportRecursive: boolean
  public miniComponents: Components
  public thirdPartyPatcher: Record<string, Record<string, string>> = {}
  public modifyCompProps?: (
    compName: string,
    target: Record<string, string>
  ) => Record<string, string>
  public modifyLoopBody?: (child: string, nodeName: string) => string
  public modifyLoopContainer?: (children: string, nodeName: string) => string
  public modifyTemplateResult?: (
    res: string,
    nodeName: string,
    level: number,
    children: string
  ) => string
  public modifyTemplateResultV2?: (
    res: string,
    nodeName: string,
    comp: Component,
    level: number,
  ...
  public modifyThirdPartyLoopBody?: (child: string, nodeName: string) => string
  ...
  buildAttribute(attrs: Attributes, nodeName: string): string {
    return Object.keys(attrs)
      .map(
        (k) =>
          `${k}="${k.startsWith('bind') || k.startsWith('on') || k.startsWith('catch') ? attrs[k] : `{${this.getAttrValue(attrs[k], k, nodeName)}}`}" `
      )
      .join('')
  }
  ...
  buildBaseTemplate() {
    ...
    const data =
      !this.isSupportRecursive && this.isUseXS
    ...
    return `${this.buildXsImportTemplate()}<template name="tmpl">
...
`
  ...
  buildComponentTemplate(comp: Component, level: number) {
  ...
  getChildrenTemplate(level: number, useSlotItem = false) {
    const {
      isSupportRecursive,
      isUseXS,
      Adapter,
      isUseCompileMode = true,
    } = this
  ...
  getChildren(comp: Component, level: number): string {
  ...
  buildFocusComponentTemplate(comp: Component, level: number) {
...
`
  ...
  buildStandardComponentTemplate(comp: Component, level: number) {
    const children = this.getChildren(comp, level)
    const nodeAlias = comp.nodeAlias
...
`
    ...
    if (isFunction(this.modifyTemplateResultV2)) {
      res = this.modifyTemplateResultV2(res, nodeName, comp, level, children)
    }
...
`
  ...
  buildThirdPartyTemplate(level: number, componentConfig: ComponentConfig) {
...
<template name="tmpl_${level}_${compName}">
  ...
  buildContainerTemplate(level: number) {
...
`
  ...
  getAttrValue(value: string, _key: string, _nodeName: string) {
    return `{${value}}`
  }
  ...
  public buildPageTemplate = (
    baseTempPath: string,
    _page?: { content: Record<string, any>; path: string }
  ) => {
    const template = `<import src="${baseTempPath}"/>
<template is="tmpl" data="{{${this.dataKeymap('root:root')}}}" />`
...
export class UnRecursiveTemplate extends BaseTemplate {
  ...
  buildFloor(level: number, components: string[], restart = false) {
  ...
  buildOptimizeFloor(level: number, components: string[], restart = false) {
    if (restart) return this.buildContainerTemplate(level)
